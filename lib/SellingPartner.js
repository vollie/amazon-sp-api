...
class SellingPartner {
  ...
  async callAPI(req_params) {
    ...
    if (json_res.errors?.length) {
      ...
      if (res.statusCode === 403 && error.code?.match(/^unauthori[sz]ed$/i) && /access token.*expired/i.test(error.details)) {
        if (this._options.auto_request_tokens) {
          if (this._options.debug_log) {
      ...
      } else if (res.statusCode === 429 && error.code?.match(/QuotaExceeded/i)) {
        if (this._options.auto_request_throttled) {
          return await this._retryThrottledRequest(req_params, res);
        } else {
          const headers = res?.headers || {};
          const rlHeader = Object.keys(headers).find(h => h.match(/x-amzn-ratelimit-limit/i));
          throw new CustomError({
            code: 'QUOTA_EXCEEDED',
            message: error.details || error.message,
